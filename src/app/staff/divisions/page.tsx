...
export default function ManageDivisions() {
  ...
  const loadDivisions = async (deptId: string) => {
    ...
    const withCounts = await Promise.all(divs.map(async (d: any) => {
      const [{ count: staffCount }, { count: unitCount }, { count: projectCount }] = await Promise.all([
        supabase.from('profiles').select('id', { count: 'exact', head: true }).eq('division_id', d.id),
        supabase.from('units').select('id', { count: 'exact', head: true }).eq('division_id', d.id),
        supabase.from('projects').select('id', { count: 'exact', head: true }).eq('div_scope_id', d.id),
      ]);
      return { ...d, staffCount: staffCount ?? 0, unitCount: unitCount ?? 0, projectCount: projectCount ?? 0 };
    }));
  ...
  useEffect(() => {
    if (!myDept?.id) return;
    if (headSearch.length < 2) { setSearchResults([]); return; }
  ...
  }, [headSearch, myDept?.id]);

  const handleSelectHead = useCallback((staff: any) => {
  ...
  const handleUpdate = async () => {
    ...
    const { error: err } = await supabase.from('divisions').update({
      name: editName.trim(),
      code: editCode.trim() || null,
      description: editDescription.trim() || null,
      head_id: selectedHead?.id || null, // Updated to allow unassigning
    ...
    if (selectedHead?.id) {
      await supabase.from('profiles').update({
        role: 'DIVISION_HEAD', division_id: selectedDivision.id, department_id: myDept.id,
      }).eq('id', selectedHead.id);
    }

    resetForm(); setShowManageModal(false);
    await loadDivisions(myDept.id);
    setSubmitting(false);
  };
  ...
  if (loading) return (
    <div className="div-loading"><div className="div-loading-bar" /><span>Loading divisions…</span></div>
  );
  ...
  return (
    <div className="div-page">
      ...
      {divisions.length === 0 ? (
        <div className="div-empty"><p>No divisions yet. Create one to get started.</p></div>
      ) : (
        <div className="div-grid">
          {divisions.map((div) => (
            <div key={div.id} className="div-card" onClick={() => router.push(`/staff/divisions/${div.id}`)}>
              <div className="div-card-top">
                ...
                <button className="div-manage-btn" onClick={(e) => { e.stopPropagation(); openManage(div); }}>
                  Manage
                </button>
              </div>
              {div.description && <p className="div-card-desc">{div.description}</p>}
        ...
        <div className="div-overlay" onClick={() => setShowCreateModal(false)}>
          <div className="div-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="div-form-group">
              ...
              <textarea className="div-input" rows={3}
                value={newDescription} onChange={(e) => setNewDescription(e.target.value)} />
            </div>
            <div className="div-form-group" style={{ position: 'relative' }}>
              <label>Division Head (optional)</label>
              {selectedHead ? (
                <SelectedHead head={selectedHead} onRemove={() => { setSelectedHead(null); setHeadSearch(''); }} />
              ) : (
                <SearchDropdown
                  headSearch={headSearch} setHeadSearch={setHeadSearch}
      ...
      {showManageModal && selectedDivision && (
        <div className="div-overlay" onClick={() => { setShowManageModal(false); resetForm(); }}>
          <div className="div-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="div-form-group">
              <label>Division Name *</label>
              <input className="div-input" value={editName} onChange={(e) => setEditName(e.target.value)} />
            </div>
            <div className="div-form-group">
              <label>Division Code</label>
              <input className="div-input" value={editCode} onChange={(e) => setEditCode(e.target.value)} />
            </div>
            <div className="div-form-group">
              <label>Description</label>
            ...
            <div className="div-modal-actions div-modal-actions-split">
              <div>
                {!confirmDelete ? (
                  <button className="div-btn-danger-outline" onClick={() => setConfirmDelete(true)}>🗑 Delete Division</button>
                ) : (
